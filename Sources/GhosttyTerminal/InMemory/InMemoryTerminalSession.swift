...
public final class InMemoryTerminalSession: @unchecked Sendable {
    ...
    // MARK: - Screen Read (viewport + scrollback)

    /// Returns the full screen — scrollback history *and* the active viewport
    /// — as a UTF-8 string, or `nil` if no surface is attached. Identical
    /// `ghostty_text_s` lifecycle and locking as `readViewportText()`; only
    /// the selection coordinate space differs.
    ///
    /// Selection grammar: `(SCREEN, TOP_LEFT)` to `(SCREEN, BOTTOM_RIGHT)`
    /// with `rectangle: false`. `GHOSTTY_POINT_SCREEN` spans the entire
    /// scrollback + viewport; `BOTTOM_RIGHT` clamps to the last written cell,
    /// so no explicit row count is needed and empty history degrades to a
    /// viewport-only result.
    ///
    /// Added downstream (btty `lulu-code-bzs`, Phase 5.5 full-scrollback
    /// search): upstream exposes only the viewport read, but the C API
    /// (`ghostty_surface_read_text` + `GHOSTTY_POINT_SCREEN`) supports this.
    ///
    /// Drains pending host writes first so a read immediately after
    /// `receive(_:)` reflects them (`receive` became async upstream in
    /// e10271c — writes enqueue on the surface-access serial queue).
